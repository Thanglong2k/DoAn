...
namespace Infrastructure
{
    public class BaseRepository<TEntity> : IBaseRepository<TEntity>, IDisposable where TEntity : BaseEntity // điều kiện thằng con kế thừa phải là class
    {
        ...
        /// </returns>
        /// CreatedBy: TTLONG (28/7/2021)
        public OutputFilterPaging<TEntity> GetAllPaging(int pageIndex, int pageSize, string queryText)
        {
            try
            {
                ...
                //Khởi tạo các CommandText:
                var entitites = DbConnection.Query<TEntity>($"Proc_Get{_className}sPaging", parameters, commandType: CommandType.StoredProcedure);

                var output = new OutputFilterPaging<TEntity>();
        ...
        public virtual int Delete(Guid entityId)
        {
            ...
            using (var transaction = DbConnection.BeginTransaction())
            {
                try
                {
                    ...
                    rowAffects = DbConnection.Execute($"Proc_Delete{_className}", parameters, transaction: transaction, null, CommandType.StoredProcedure);
        ...
        internal DynamicParameters MappingDbType(TEntity entity)
        {
            ...
            foreach (var property in properties)
            {
                if (property.Name != "EntityState")
                {
                    ...
                    else if (propertyType == typeof(List<ProductAttribute>))
                    {
                        parameters.Add($"@{propertyName}", propertyValue, DbType.String);
                    }
                    else if (propertyType == typeof(IFormFile) || propertyType == typeof(List<IFormFile>))
                    {
                        parameters.Add($"{propertyName}", propertyValue, DbType.String);
                    }
                    else
